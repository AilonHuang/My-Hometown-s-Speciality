...
function zsc_show(id) {
    ...
    if (zsc_class != 'add') {
        ...
        click_id = id;
        var zsc_id = id;
        var url = "/shop/index.php/admin/keyword/addkeyword_ajax/id/" + zsc_id;
        $.post(url, null, function (data) {
            $.each(data, function (i, v) {
                ...
                zsc_html = "<div class=\"edit-info\">" +
                        ...
                        "<img id=\"zsc_imgs0\" src=\"/shop/Public/weixin/images/dingcan/bigimg.jpg\" width=\"295\" height=\"126\"/>" +
                        "<span id=\"zsc_titles0\">标题</span>" +
                        "<b class=\"modify\"><img id=\"zsc_simg0\"" +
                        " src=\"/shop/Public/weixin/images/dingcan/op-modify.png\" onclick=\"zsc_editimg(0)\"/></b>" +
                        ...
                        "<img src=\"/shop/Public/weixin/images/dingcan/keywords_28.jpg\" class=\"add1\" " +
                        "style=\"cursor:pointer;\" onclick=\"zsc_addimg(1)\" />" +
                        "</div>" +
                        "</div>" +
                        "<div class=\"tuwen_right\">" +
                        "<img src=\"/shop/Public/weixin/images/dingcan/keywords_15.jpg\">" +
                        "<p>" +
                        "<br />" +
                        ...
                        "</p>" +
                        "<img src=\"/shop/Public/weixin/images/dingcan/keywords_19.jpg\">" +
                        "</div>" +
                        "<div style=\" padding-top:45px; clear:both;\">" +
                ...
                $('#zsc_kname').val(v.name);
                $('#zsc_kword').val(v.keyword);
                if (v.type == 1) {
                    check_tuwen(1);
...
function zsc_addimg(num) {
    if ($('.simgcon').length < 9) {
        ...
        var zsc_html = "<a href=\"javascript:;\" class=\"simgcon\" id=\"zsc_imgs" + num + "\">" +
                "<span>标题</span>" +
                "<img src=\"/shop/Public/weixin/images/dingcan/smallimg.jpg\" id=\"zsc_simg" + num + "\" />" +
                "<b class=\"smodify\"><img src=\"/shop/Public/weixin/images/dingcan/op-modify.png\" onclick=\"zsc_editimg(" + num + ")\" />" +
                "<img src=\"/shop/Public/weixin/images/dingcan/op-del.png\" onclick=\"zsc_delimg(" + num + ")\" /></b>" +
...
function zsc_editimg(num) {
    ...
    if (0 == zsc_count) {
        $('#zsc_titles').val($('#zsc_titles0').html());
        $('#zsc_urls').val($('#zsc_urls0').val());
        if ('/shop/Public/weixin/images/dingcan/bigimg.jpg' == $('#zsc_imgs0').attr('src')) {
            $('#zsc_imgurls').val();
        } else {
        ...
        if ('/shop/Public/weixin/images/dingcan/smallimg.jpg' != ssurl) {
...
function choseImg() {
    ...
    $('.zhe').show();
    $('.imgbox a').remove();
    var url = "/shop/index.php/admin/keyword/allimages";
    $.post(url, null, function (data) {
        $.each(data, function (i, v) {
            $('.imgbox').append("<a href=\"javascript:choseImages('" + v.id + "');\"><img src=\"/shop/public/uploads/" + v.imgurl + "\" id=\"zsc_imgid" + v.id + "\"/><span onclick=\"delImg('" + v.id + "')\">X</span></a>");
        });
    }, 'json');
}
...
function delImg(num) {
    ...
    var url = "/shop/index.php/admin/keyword/delimg";
...
function btnsave(num, count) {
    ...
    if (ktype == 1) {
        ...
        if (count == 0) {
            var url = "/shop/index.php/admin/keyword/addkeyword/op/add";
            $.post(url, {kename: kname, keword: kword, kecontent: kecontent, ketype: ktype}, function (data) {
                alert('添加成功！');
                window.location.reload();
            });
        } else {
            var url = "/shop/index.php/admin/keyword/addkeyword/op/update";
            var kid = $('#zsc_iid').val();
...
function zscdel(num) {
    ...
    var url = "/shop/index.php/admin/keyword/addkeyword/op/del";
