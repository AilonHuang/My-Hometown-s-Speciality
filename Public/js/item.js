    ...
    $.pinphp.item = {
        ...
        like: function(){
            ...
            $(s.like_btn).live('click', function(){
                ...
                var self = $(this),
                    ...
                    n = parseInt(nb.html());
                $.getJSON(PINER.root + '/?m=Goods&a=like', {id:id, aid:aid}, function(result){
                    if(result.status == 1){
                        //喜欢成功
                        nb.text(n+1).show();
        ...
        unlike: function(){
            ...
            $(s.unlike_btn).live('click', function(){
                ...
                var self = $(this),
                    id = self.attr('data-id');
                $.getJSON(PINER.root + '/?m=Goods&a=unlike', {id:id}, function(result){
                    if(result.status == 1){
        ...
        del: function(){
            ...
            $(s.del_btn).live('click', function(){
                ...
                $.getJSON(PINER.root + '/?m=Goods&a=delete', {id:id, album_id:album_id}, function(result){
                    if(result.status == 1){
                        self.parents(s.item_unit).slideUp(500, function(){
                            $(this).parent().masonry('remove', $(this)).masonry('reload');
                        });
        ...
        share: function(){
            ...
            $(s.share_btn).live('click', function(){
                ...
                $.getJSON(PINER.root + '/?m=Goods&a=share_item', function(result){
                    if(result.status == 1){
        ...
        share_form: function(form, aid){
            form.find('.J_si_btn').die('click').live('click', function(){
                ...
                $.getJSON(PINER.root + '/?m=Goods&a=fetch_item', {url:si_url, aid:aid}, function(result){
                    if(result.status == 1){
        ...
        fetch_form: function(form, item){
            form.find('.J_pub_btn').die('click').live('click', function(){
                ...
                $.ajax({
                    url: PINER.root + '/?m=Goods&a=publish_item',
                    type: 'POST',
                    data: {
                        item: item,
                        album_id: album_id,
