    ...
    <div class="J_tablelist table_list" data-acturi="<?php echo U('item_cate/ajax_edit');?>">
        <table width="100%" cellspacing="0" id="J_cate_tree">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr id='node-340'>
                    ...
                    <td><span data-tdtype='edit' data-field='name' data-id='340' class='tdedit' style='color:'><?php echo ($val["type_name"]); ?></span>
                    </td>


                    <td align='center'>
                        <a href="<?php echo U('Attribute/index',array('type_id'=>$val['id']));?>" class="" data-uri="" data-title="" data-id="add"
                           data-width="520" data-height="360">属性列表</a>|
                        <a href="<?php echo U('Type/edit',array('id'=>$val['id']));?>" class=""
                           data-uri="" data-title=""
                           data-id="edit" data-width="520" data-height="360">编辑</a> |
                        <a onclick="return confirm('确定要删除吗？')" href="<?php echo U('Type/delete',array('id'=>$val['id']));?>" class="" data-acttype="ajax"
