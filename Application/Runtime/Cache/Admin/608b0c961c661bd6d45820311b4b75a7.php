...
<form id="info_form" action="<?php echo u('Goods/edit');?>" method="post" enctype="multipart/form-data">
	<div class="pad_lr_10">
		<div class="col_tab">
			...
			<div class="J_panes">
						...
						<tr>
							...
							<td>
								<select name="cat_id" class="J_cate_select mr10" data-pid="0" data-uri="<?php echo U('item_cate/ajax_getchilds', array('type'=>0));?>" data-selected="">
									<?php if(is_array($catData)): $i = 0; $__LIST__ = $catData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option <?php if($val["id"] == $data['cat_id']): ?>selected="selected"<?php endif; ?> value="<?php echo ($val["id"]); ?>"><?php echo ($val["cat_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
								</select>
							</td>
						</tr>
						<tr>
							<th>商品名称 :</th>
							<td><input type="text" name="goods_name" id="J_title" class="input-text" size="60" value="<?php echo ($data["goods_name"]); ?>"></td>
						</tr>
						<tr>
							<th>商品简介 :</th>
							<td><textarea name="goods_intro" cols="80" rows="2"><?php echo ($data["goods_intro"]); ?></textarea></td>
						</tr>
						<tr>
							<th>商品图片 :</th>
							<td><input type="file" name="img" /><img src="<?php echo showImage($data['sm_img']);?>" alt=""></td>

						</tr>

						<tr>
							<th>商品价格 :</th>
							<td><input value="<?php echo ($data["price"]); ?>" id='J_price' onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')" onafterpaste="this.value=this.value.replace(/[^0-9.]/g,'')" type="text" name="price" class="input-text" size="10"> 元</td>
						</tr>
						<tr>
							<!--<th>是否推荐:</th>--><td></td>

							<td><input <?php if($data["is_new"] == 1): ?>checked="checked"<?php endif; ?> type="checkbox" name="is_new" value="1">新品&nbsp;
								<input <?php if($data["is_hot"] == 1): ?>checked="checked"<?php endif; ?> type="checkbox" name="is_hot" value="1">热卖
				...
				<div class="content_list pad_10 hidden">
					<table width="100%" cellpadding="2" cellspacing="1" class="table_form" id="first_upload_file">
						<tr>
							<ul id="img_ul">
								<?php if(is_array($goodsImgData)): $i = 0; $__LIST__ = $goodsImgData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li><input img_id="<?php echo ($val["id"]); ?>" class="delimage" type="button" value="删除"><br>
										<img src="<?php echo showImage($val['sm_img']);?>" alt=""></li><?php endforeach; endif; else: echo "" ;endif; ?></ul>
						</tr>
						<tbody class="uplode_file">
						<tr>
							<th width="100"><a href="javascript:void(0);" class="blue" onclick="add_file();"><img src="/shop/Public/css/admin/bgimg/tv-expandable.gif" /></a> 上传图片 :</th>
				...
				<div class="content_list pad_10 hidden">
					<table width="100%" cellpadding="2" cellspacing="1" class="table_form" id="item_attr">
						...
						<?php if(is_array($attrData)): $i = 0; $__LIST__ = $attrData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                                <td><?php echo ($val["attr_name"]); ?>:
                                   <?php if($goodsAttrData):?>
                                       <?php foreach($goodsAttrData as $k=>$v): ?>
                                            <?php if($k == $val['id']):?>
                                                <input value='<?php echo ($v); ?>' type="text" name="attr_id[<?php echo ($val['id']); ?>]">
                                            <?php endif;?>
                                        <?php endforeach; ?>
                                    <?php else:?>
                                        <input value='<?php echo ($v); ?>' type="text" name="attr_id[<?php echo ($val['id']); ?>]">
...
<script>
	...
	$(".delimage").click(function(){
	   if(confirm('确定要删除吗？')){
	       ...
	       $.ajax({
			   type:'GET',
			   url:"<?php echo U('Goods/ajaxDelImage','',FALSE);?>/img_id/"+img_id,
